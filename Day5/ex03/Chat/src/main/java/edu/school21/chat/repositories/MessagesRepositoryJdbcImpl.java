...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public Optional<Message> findById(Long id) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            System.out.println("Message : {\n id=" + messageID + ",\n " + findUserById( authorID) + ",\n " + findChatroomById( chatroomID) + ",\n text=\"" + text + "\",\n datetime=" + datetime.replace('-', '/').substring(2, 16) + "\n}");
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
            return Optional.of(new Message(messageID, findUserById(authorID), findChatroomById(chatroomID), text, LocalDateTime.parse(datetime, formatter)));
        }
    ...
    public void update(Message message) throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement("update chat.messages set authorid=?, chatroomid=?, text=?, datetime=? where id=?  ");
        if(message.getCreator() != null || findUserById(message.getCreator().getId()) != null) {
            preparedStatement.setInt(1, message.getCreator().getId());
        }
        else{
